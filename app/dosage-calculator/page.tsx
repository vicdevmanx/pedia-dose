...
export default function DosageCalculatorPage() {
  ...
  const calculateDosage = () => {
    ...
    setTimeout(() => {
      // Extract dosage range from mg/kg string (e.g., "10-15 mg/kg" -> [10, 15])
      ...
      // Determine recommended dose (prefer weight-based for most pediatric drugs)
  ...
  const generatePrescription = () => {
    ...
    // In real app, this would create a prescription record
    ...
    // Redirect to prescriptions page or show success message
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Dosage Calculator</h1>
            <p className="text-muted-foreground">Calculate safe pediatric dosages with real-time safety checks</p>
          </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div
                className={`flex items-center gap-4 cursor-pointer ${currentStep >= 1 ? "hover:bg-muted/50 rounded-lg p-2 -m-2" : ""}`}
                onClick={() => goToStep(1)}
              >
                {getStepIcon(1)}
                <div className={currentStep >= 1 ? "text-foreground" : "text-muted-foreground"}>
                  <p className="font-semibold">Select Patient</p>
                  <p className="text-sm">Choose patient from database</p>
                </div>
              </div>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
              <div
                className={`flex items-center gap-4 cursor-pointer ${currentStep >= 2 ? "hover:bg-muted/50 rounded-lg p-2 -m-2" : ""}`}
                onClick={() => goToStep(2)}
              >
                {getStepIcon(2)}
                <div className={currentStep >= 2 ? "text-foreground" : "text-muted-foreground"}>
                  <p className="font-semibold">Choose Drug</p>
                  <p className="text-sm">Select medication to prescribe</p>
                </div>
              </div>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
              <div
                className={`flex items-center gap-4 cursor-pointer ${currentStep >= 3 ? "hover:bg-muted/50 rounded-lg p-2 -m-2" : ""}`}
                onClick={() => goToStep(3)}
              >
                {getStepIcon(3)}
                <div className={currentStep >= 3 ? "text-foreground" : "text-muted-foreground"}>
                  <p className="font-semibold">Calculate Dosage</p>
                  <p className="text-sm">System calculates safe dose</p>
                </div>
              </div>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
              <div className="flex items-center gap-4">
                ...
                <div className={currentStep >= 4 ? "text-foreground" : "text-muted-foreground"}>
                  ...
                  <p className="text-sm">Generate prescription</p>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        {currentStep === 1 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Choose a patient from the database or add a new patient</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="text-center">
                <p className="text-sm text-muted-foreground mb-2">Patient not in database?</p>
        ...
        {currentStep === 2 && selectedPatient && (
          <div className="space-y-4">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">Selected Patient</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="flex items-center justify-between">
                ...
                {selectedPatient.allergies.length > 0 && !selectedPatient.allergies.includes("None") && (
                  <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg dark:bg-yellow-950 dark:border-yellow-800">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-yellow-800 dark:text-yellow-200">Known Allergies:</span>
                    </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Select the medication to prescribe</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <DrugSearch
                  onDrugSelect={(drug) => {
          ...
          <div className="space-y-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Select administration route and calculate safe dosage</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="p-4 bg-muted rounded-lg">
                        <p className="text-2xl font-bold text-primary">{dosageResult.weightBased} mg</p>
                        <p className="text-sm text-muted-foreground">
                    ...
                    {dosageResult.bsaBased && (
                      <div>
                        ...
                        <div className="p-4 bg-muted rounded-lg">
                          <p className="text-2xl font-bold text-secondary">{dosageResult.bsaBased} mg</p>
                          <p className="text-sm text-muted-foreground">Based on body surface area</p>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      <h4 className="font-semibold mb-2">Recommended Dose</h4>
                    ...
                    <div>
                      ...
                      <div className="p-4 bg-red-50 border border-red-200 rounded-lg dark:bg-red-950 dark:border-red-800">
                        <p className="text-lg font-bold text-red-700 dark:text-red-300">{dosageResult.maxDaily} mg</p>
                        <p className="text-sm text-red-600 dark:text-red-400">Do not exceed in 24 hours</p>
                      </div>
                    </div>
                  </div>
                </div>
